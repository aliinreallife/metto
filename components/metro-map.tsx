...
export function MetroMap({
  ...
  const routeEdgeKeys = useMemo(() => {
    ...
    if (route)
      for (const h of route.hops)
        set.add([h.from, h.to].sort().join("|") + "|" + h.line);
    return set;
  }, [route]);
  ...
  useEffect(() => {
    ...
    const scale = Math.min(
      MAX_SCALE,
      Math.max(MIN_SCALE, Math.min(1000 / w, 1000 / h)),
    );
  ...
  return (
    <div className="relative size-full overflow-hidden rounded-xl border border-border bg-card">
      ...
      >
        <g transform={`translate(${t.tx} ${t.ty}) scale(${t.scale})`}>
          ...
          {edges.map((e, i) => {
            ...
            if (!a || !b) return null;
            const inRoute = routeEdgeKeys.has(
              [e.a, e.b].sort().join("|") + "|" + e.line,
            );
            return (
              <line
                key={i}
          ...
          {STATIONS.map((s) => {
            ...
            const r =
              ((interchange ? 5.5 : 3.8) / Math.sqrt(t.scale)) *
              (isEndpoint ? 1.6 : 1);
            const stroke = interchange
              ? "var(--foreground)"
              : LINE_COLORS[s.lines[0]];
            return (
              <g
                key={s.id}
                opacity={dim ? 0.25 : 1}
      ...
      <div className="absolute bottom-3 right-3 flex flex-col gap-1.5">
        ...
        <MapBtn
          label="Zoom out"
          onClick={() => zoomAt({ x: 500, y: 500 }, 1 / 1.3)}
        >
